...
import type { LeadStatus, LeadPriority, Lead } from '../../types/database';
import { ArrowLeft, AlertCircle, Loader2, Bold, Italic, Code, List } from 'lucide-react';
...
export const EditPage: React.FC<EditPageProps> = ({ leadId, onNavigate }) => {
  ...
  useEffect(() => {
    const fetchLead = async () => {
      try {
        const data = await dbClient.getLeadById(leadId);
        if (!data) {
        ...
        setCompanyType(data.company_type || 'Enterprise');
        setIndustry(data.industry || '');
        setServiceNeeded(data.service_needed || '');
        setStatus(data.status);
        setPriority(data.priority);
        setNotes(data.notes || '');
        setReplyReceived(data.reply_received || '');
        setProposalSent(data.proposal_sent || '');
        setAssignedUsers(data.assigned_users || []);
        setRefImage(data.ref_image || '');
      } catch (err: any) {
  ...
  const handleAssignedToggle = (userId: string) => {
    ...
    if (lead && userId === lead.created_by) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dbClient.updateLead(leadId, {
  ...
  if (error && !lead) {
    return (
      <div className="p-6 max-w-lg mx-auto border border-border bg-card rounded shadow-sm text-center">
        <AlertCircle className="h-8 w-8 text-danger mx-auto mb-2" />
        <h3 className="font-bold text-foreground">Access Error</h3>
        <p className="text-sm text-muted-foreground mt-2">{error}</p>
        <button
          onClick={() => onNavigate('leads')}
          className="mt-4 px-4 py-2 text-xs font-semibold bg-primary text-primary-foreground rounded hover:bg-primary/95 transition-colors"
        >
          Return to Projects
        </button>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground mb-4">
              Heads can assign any teammate. The project creator is always assigned.
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {profiles
                ...
                .map((p) => {
                  ...
                  const isCreator = lead?.created_by === p.id;
                  const isHead = currentUser?.role === 'head';
                  return (
                    <button
                      key={p.id}
                      type="button"
                      disabled={isCreator || (!isHead && currentUser?.id !== p.id)}
                      ...
                      } ${(isCreator || (!isHead && currentUser?.id !== p.id)) ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'}`}
                    >
                      {isAssigned && <span className="h-1.5 w-1.5 rounded-full bg-primary" />}
                      <span>{p.full_name}</span>
                      {isCreator && <span className="text-[9px] uppercase opacity-65 font-bold ml-1">(Creator)</span>}
                    </button>
                  );
                })}
            </div>
